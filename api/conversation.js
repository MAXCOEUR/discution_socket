...
function isAdmin(token, id_conversation, parametre, func) {
...
router.get('', [
  ...
  const query = "select c.*,COALESCE(cc.unRead, 0) AS unRead " +
    ...
    "left join ( SELECT m.id_conversation, COUNT(m.id) - IFNULL(COUNT(r.id_message), 0) AS unread " +
    "FROM messages m " +
...
const putConversation = function (parametre) {
